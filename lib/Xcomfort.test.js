...
describe('setDimState', () => {
  ...
  it('should reject when device type not DimActuator', (done) => {
      // New mocks
      mockX.query = jest.fn();

      setDimState('dev','on')
        .catch( (reason) => {
          expect(reason).toBe('No dimmable device with that name exists');

          // Test that callback also works on rejection
          setDimState('dev','on', (err) => {
            expect(err).toBe('No dimmable device with that name exists');
            // query should not be called
            expect(mockX.query.mock.calls.length).toBe(0);
            done();
          });
