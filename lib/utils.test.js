...
describe('invokeDCOOperation', () => {
  ...
  const invokeDCOOperation = utils.invokeDCOOperation.bind(mockX);

  it('should call query with correct arguments', (done) => {

    invokeDCOOperation(testDevice.id, testDevice.type, 'test', { ok: 'ok'})
      .then( () => {
        expect(mockX.query.mock.calls[0][0])
          .toBe('HDAccess/invokeDCOOperation');
        expect(mockX.query.mock.calls[0][1])
          .toEqual([
            'hdm:xComfort Adapter:2343245_o',
            'com.eaton.xcomfort.hdm.dc.' + testDevice.type,
            'test',
            { ok: 'ok'}
          ]);
