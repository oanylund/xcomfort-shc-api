 ...
 * @param  {string} importPath - Relative path to file
 * @emits module:Xcomfort#ready
 * @emits module:Xcomfort#error
 * @alias module:Xcomfort~importSetup
 */
init.importSetup = function (importPath) {
  if(typeof importPath !== 'string') {
    throw new Error('importPath is not a string');
  }

  utils.readJsonFile(importPath)
    .then( (importObj = {}) => {
      if(!importObj.devices) {
        throw new Error('Parsed object is missing devices property');
      }
      if(!importObj.scenes) {
        throw new Error('Parsed object is missing scenes property');
      }

      this.deviceMap = new Map(importObj.devices);
      this.sceneMap = new Map(importObj.scenes);
